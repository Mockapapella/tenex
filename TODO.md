...
- [x] #110 Stale PTY size after restart on a smaller screen. Resize now means exactly the requested size, with zero dimensions rejected at the boundary, resize failures surfaced instead of swallowed, and the last-resize-wins multi-client contract documented and pinned by test. Merged in #151. (S)
...
- [x] #112 Subagents lack full scrollback. Paused/scrolled previews now render from the daemon's canonical full history instead of the bounded stream cache; the #113 request-count regression guards landed alongside. Merged in #152. (S)
...
- [x] #12 Selective synthesis. Mark agents in the sidebar; `s` synthesizes marked descendants and falls back to all when nothing is marked. Marked nodes target their whole subtree, the confirmation count and execution share one target-selection helper, and surviving siblings get their mux window indices adjusted after selective kills. Merged in #155. (M)
