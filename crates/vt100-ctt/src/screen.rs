...
impl Screen {
    pub(crate) fn new(
        ...
        Self {
            grid,
            // Tenex uses vt100 as a *preview* buffer for agent output. For full-screen TUIs
            // (like Codex) the alternate screen is where most output lives, so keep scrollback
            // there too so the preview can be scrolled.
