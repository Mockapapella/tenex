...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // Tenex uses scrollback to render a scrollable preview of agent output.
            // Many full-screen TUIs (including Codex) scroll inside a scroll region,
            // and still expect those lines to be viewable later. Preserve them.
