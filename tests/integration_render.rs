...
#![expect(clippy::needless_raw_string_hashes, reason = "test string readability")]
#![expect(clippy::uninlined_format_args, reason = "test readability")]
#![expect(clippy::single_char_pattern, reason = "test clarity")]
#![expect(clippy::needless_borrows_for_generic_args, reason = "test clarity")]
...
fn test_agent_list_widget_renders() {
    ...
    let widget = AgentListWidget::new(&agents, 0);
...
fn test_agent_list_widget_with_selection() {
    let agents = create_test_agents();
    let widget = AgentListWidget::new(&agents, 1); // Select second agent
...
fn test_agent_list_widget_custom_title() {
    let agents = create_test_agents();
    let widget = AgentListWidget::new(&agents, 0).title("Custom Title");
...
fn test_preview_widget_renders_content() {
    let content = "Line 1\nLine 2\nLine 3\nLine 4\nLine 5";
    let widget = PreviewWidget::new(content);
...
fn test_preview_widget_with_scroll() {
    let content = "Line 1\nLine 2\nLine 3\nLine 4\nLine 5\nLine 6\nLine 7\nLine 8";
    let widget = PreviewWidget::new(content).scroll(3);
...
#[test]
fn test_full_render_normal_mode() {
    ...
    terminal
        .draw(|frame| {
            ...
            match app.active_tab {
                Tab::Preview => {
                    let preview =
                        PreviewWidget::new(&app.preview_content).scroll(app.preview_scroll);
...
#[test]
fn test_full_render_creating_mode() {
    let backend = TestBackend::new(80, 24);
    let mut terminal = Terminal::new(backend).unwrap();
    ...
    terminal
        .draw(|frame| {
            ...
            let status = StatusBarWidget::status(&format!("Enter name: {}", app.input_buffer));
            frame.render_widget(
                status.to_paragraph(),
...
#[test]
fn test_full_render_confirming_mode() {
    let backend = TestBackend::new(80, 24);
    let mut terminal = Terminal::new(backend).unwrap();
    let mut app = create_test_app_with_agents();
...
#[test]
fn test_full_render_with_error() {
    let backend = TestBackend::new(80, 24);
    let mut terminal = Terminal::new(backend).unwrap();
    let mut app = create_test_app_with_agents();
