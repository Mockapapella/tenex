...
fn codex_home_dir(home: &Path) -> PathBuf {
    #[cfg(test)]
    let override_path = codex_home_override_store()
        .read()
...
#[cfg(test)]
fn codex_home_override_store() -> &'static RwLock<Option<PathBuf>> {
...
#[cfg(test)]
pub(super) fn with_docker_runtime_overrides_for_tests<T>(
    program: PathBuf,
    ...
    *codex_home_override_store()
        .write()
        .unwrap_or_else(std::sync::PoisonError::into_inner) = codex_home;
    f()
}
...
pub(super) fn with_docker_program_override_for_tests<T>(
    ...
    with_docker_runtime_overrides_for_tests(program, None, f)
}

#[cfg(test)]
pub(super) fn with_docker_program_and_codex_home_override_for_tests<T>(
    program: PathBuf,
    codex_home: PathBuf,
    f: impl FnOnce() -> T,
) -> T {
    with_docker_runtime_overrides_for_tests(program, Some(codex_home), f)
}
