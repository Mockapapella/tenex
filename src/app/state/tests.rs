...
fn test_select_next() {
    ...
    app.data.storage.add(create_test_agent("agent1"));
...
fn test_enter_exit_mode() {
    ...
    app.data.input.buffer.push_str("test");
    app.exit_mode();
    assert_eq!(app.mode, AppMode::normal());
    assert!(app.data.input.buffer.is_empty());
}
...
fn test_error_handling() {
    ...
    assert_eq!(app.data.ui.last_error, Some("Test error".to_string()));
...
fn test_status_handling() {
    ...
    assert_eq!(app.data.ui.status_message, Some("Test status".to_string()));
...
fn test_handle_backspace() {
    ...
    assert_eq!(app.data.input.cursor, 3);
...
fn test_increment_child_count() {
    let mut app = App::default();
    assert_eq!(app.data.spawn.child_count, 3);
    app.increment_child_count();
    assert_eq!(app.data.spawn.child_count, 4);
}
...
fn test_decrement_child_count() {
    ...
    app.decrement_child_count();
    assert_eq!(app.data.spawn.child_count, 1); // Minimum is 1
}
...
fn test_start_spawning_under() {
    ...
    assert_eq!(app.data.spawn.spawning_under, Some(id));
    assert_eq!(app.data.spawn.child_count, 3);
...
fn test_input_cursor_up_down() {
    let mut app = App::default();
    app.data.input.buffer = "line1\nline2\nline3".to_string();

    // Start at end of line2
    app.data.input.cursor = 11; // "line1\nline2" length

    // Move up to line1
    app.input_cursor_up();
    assert_eq!(app.data.input.cursor, 5); // End of "line1"
    ...
    assert_eq!(app.data.input.cursor, 11); // End of "line1\nline2"

    // Move down to line3
    app.input_cursor_down();
    assert_eq!(app.data.input.cursor, 17); // End of string

    // Move down from last line does nothing
    app.input_cursor_down();
    assert_eq!(app.data.input.cursor, 17);
}
...
fn test_input_cursor_home_end() {
    let mut app = App::default();
    app.data.input.buffer = "line1\nline2\nline3".to_string();
    app.data.input.cursor = 8; // Middle of "line2"

    app.input_cursor_home();
    assert_eq!(app.data.input.cursor, 6); // Start of "line2"

    app.input_cursor_end();
    assert_eq!(app.data.input.cursor, 11); // End of "line2"
...
fn test_scroll_methods() {
    ...
    app.scroll_up(1);
    assert_eq!(app.data.ui.preview_scroll, 1);
    assert!(!app.data.ui.preview_follow);

    // Test scroll_down in Preview mode
    app.scroll_down(1);
    assert_eq!(app.data.ui.preview_scroll, 2);
    ...
    assert_eq!(app.data.ui.preview_scroll, 3);
    assert!(app.data.ui.preview_follow);

    // Switch to Diff tab and test
    app.data.active_tab = Tab::Diff;
    app.data.ui.diff_scroll = 2;
    app.scroll_up(1);
    // normalize_scroll clamps to max (1 for 3 lines with 2 visible)
    assert!(app.data.ui.diff_scroll <= 1);

    app.data.ui.diff_scroll = 0;
    app.scroll_down(1);
    assert_eq!(app.data.ui.diff_scroll, 1);

    app.scroll_to_top();
    assert_eq!(app.data.ui.diff_scroll, 0);
...
fn test_selected_depth() {
    ...
    app.data.storage.add(create_test_agent("root"));
    assert_eq!(app.selected_depth(), 0);
}
...
fn test_confirm_rename_branch() {
    ...
    app.data.input.buffer = "  new-branch  ".to_string();
    assert!(app.confirm_rename_branch());
    assert_eq!(app.data.git_op.branch_name, "new-branch");
}
...
fn test_filtered_slash_commands_with_filter() {
    let mut app = App::default();
    app.data.input.buffer = "/age".to_string();
...
fn test_reset_slash_command_selection() {
    let mut app = App::default();
    app.data.command_palette.selected = 5;
...
fn test_submit_slash_command_palette_exact_match() {
    let mut app = App::default();
    app.start_command_palette();
    app.data.input.buffer = "/help".to_string();
    app.submit_slash_command_palette();
    assert_eq!(app.mode, AppMode::Help(HelpMode));
}
...
fn test_submit_slash_command_palette_prefix_match() {
    let mut app = App::default();
    app.start_command_palette();
    app.data.input.buffer = "/hel".to_string();
    app.submit_slash_command_palette();
    assert_eq!(app.mode, AppMode::Help(HelpMode));
}
...
fn test_select_prev_model_program() {
    let mut app = App::default();
    app.start_model_selector();
    app.data.model_selector.selected = 0;
...
fn test_handle_model_filter_backspace() {
    let mut app = App::default();
    app.start_model_selector();
    app.data.model_selector.filter = "abc".to_string();

    app.handle_model_filter_backspace();
    assert_eq!(app.data.model_selector.filter, "ab");
}
...
fn test_agent_spawn_command_claude() {
    ...
    assert_eq!(cmd, app.data.config.default_program);
}
...
fn test_agent_spawn_command_custom() {
    ...
    app.data.settings.custom_agent_command = "my-agent --flag".to_string();
...
fn test_agent_spawn_command_custom_empty() {
    let mut app = App::default();
    app.data.settings.agent_program = AgentProgram::Custom;
    app.data.settings.custom_agent_command = "   ".to_string();
...
fn test_start_custom_agent_command_prompt() {
    let mut app = App::default();
    app.data.settings.custom_agent_command = "my-agent".to_string();
...
fn test_confirm_model_program_selection_codex() {
    ...
    assert!(matches!(
        &app.mode,
        AppMode::Normal(_) | AppMode::ErrorModal(_)
    ));
}
