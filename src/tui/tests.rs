...
use crate::app::{
    BranchPickerKind, ConfirmAction, ConfirmKind, CountPickerKind, OverlayMode, TextInputKind,
};
            ...
            if let Ok(worktrees) = repo.worktrees() {
                for wt_name in worktrees.iter().flatten() {
                        ...
                        && let Ok(wt) = repo.find_worktree(wt_name)
                    {
                        ...
                        {
                            eprintln!("Warning: Failed to remove test worktree dir {path}: {e}");
                        }
                        if let Err(e) =
                ...
                for branch_result in branches {
                        ...
                        && name.starts_with(&self.branch_prefix)
                    {
                        ...
                        if let Err(e) = branch.delete() {
                            eprintln!("Warning: Failed to delete test branch {branch_name}: {e}");
                        }
                    }
                }
            }
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        app.mode,
        Mode::Overlay(OverlayMode::Confirm(ConfirmKind::Action(_)))
    ));
    Ok(())
}
...
fn test_handle_key_event_prompting_mode_escape() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.enter_mode(text_input_mode(TextInputKind::Prompting));
    app.handle_char('t');
    app.handle_char('e');
...
fn test_handle_key_event_confirming_kill() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.enter_mode(confirm_action_mode(ConfirmAction::Kill));
...
#[test]
fn test_handle_key_event_creating_mode_enter_with_input() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    assert!(
        matches!(app.mode, Mode::Overlay(OverlayMode::Error(_)))
            || app.mode == Mode::Normal
            || app.mode == confirm_action_mode(ConfirmAction::WorktreeConflict),
...
#[test]
fn test_handle_key_event_prompting_mode_enter_with_input() -> Result<(), Box<dyn std::error::Error>>
{
    let (mut app, _cleanup) = create_test_app_with_cleanup();
    let handler = Actions::new();
...
fn test_handle_key_event_broadcasting_mode_input() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.enter_mode(text_input_mode(TextInputKind::Broadcasting));
...
#[test]
fn test_handle_key_event_broadcasting_mode_enter_no_agent() -> Result<(), Box<dyn std::error::Error>>
{
    let mut app = create_test_app();
    let handler = Actions::new();
...
#[test]
fn test_handle_key_event_broadcasting_mode_enter_empty() -> Result<(), Box<dyn std::error::Error>> {
    let mut app = create_test_app();
    let handler = Actions::new();
...
fn test_handle_key_event_child_count_mode_enter() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.enter_mode(count_picker_mode(CountPickerKind::ChildCount));
...
fn test_handle_key_event_child_prompt_mode_input() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.enter_mode(text_input_mode(TextInputKind::ChildPrompt));
...
#[test]
fn test_handle_key_event_child_prompt_mode_enter_no_agent() -> Result<(), Box<dyn std::error::Error>>
{
    let (mut app, _cleanup) = create_test_app_with_cleanup();
    let handler = Actions::new();
...
#[test]
fn test_handle_key_event_review_info_mode_any_key_exits() -> Result<(), Box<dyn std::error::Error>>
{
    let mut app = create_test_app();
    let handler = Actions::new();
...
#[test]
fn test_handle_key_event_review_child_count_mode_up_down() -> Result<(), Box<dyn std::error::Error>>
{
    let mut app = create_test_app();
    let handler = Actions::new();

    app.enter_mode(count_picker_mode(CountPickerKind::ReviewChildCount));
    let initial_count = app.spawn.child_count;
...
#[test]
fn test_handle_key_event_review_child_count_mode_enter() -> Result<(), Box<dyn std::error::Error>> {
    let mut app = create_test_app();
    let handler = Actions::new();
...
#[test]
fn test_handle_key_event_review_child_count_mode_escape() -> Result<(), Box<dyn std::error::Error>>
{
    let mut app = create_test_app();
    let handler = Actions::new();
...
#[test]
fn test_handle_key_event_branch_selector_mode_navigation() -> Result<(), Box<dyn std::error::Error>>
{
    let mut app = create_test_app();
    let handler = Actions::new();
    ...
    app.enter_mode(branch_picker_mode(BranchPickerKind::ReviewBaseBranch));
...
#[test]
fn test_handle_key_event_branch_selector_mode_backspace() -> Result<(), Box<dyn std::error::Error>>
{
    let mut app = create_test_app();
    let handler = Actions::new();
...
fn test_handle_key_event_branch_selector_mode_enter() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        app.review.base_branch.is_some()
            || matches!(app.mode, Mode::Overlay(OverlayMode::Error(_))),
...
fn test_handle_key_event_confirm_push_mode_no() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.enter_mode(confirm_mode(ConfirmKind::Push));
...
fn test_handle_key_event_rename_branch_input() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.enter_mode(text_input_mode(TextInputKind::RenameBranch));
...
fn test_handle_key_event_confirm_push_for_pr_no() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.enter_mode(confirm_mode(ConfirmKind::PushForPR));
    app.git_op.agent_id = Some(uuid::Uuid::new_v4());
    app.git_op.branch_name = "test".to_string();
...
#[test]
fn test_handle_key_event_confirm_push_other_keys_ignored() -> Result<(), Box<dyn std::error::Error>>
{
    let mut app = create_test_app();
    let handler = Actions::new();
...
#[test]
fn test_handle_key_event_preview_focused_collects_keys() -> Result<(), Box<dyn std::error::Error>> {
    let mut app = create_test_app();
    let handler = Actions::new();
