...
fn base_handlers() -> KeyEventHandlers {
    ...
    let ok_changelog: fn(&mut App, Option<Version>, usize, KeyCode, KeyModifiers) -> Result<()> =
        |_, _, _, _, _| Ok(());
    let ok_app_string: fn(&mut App, String) -> Result<()> = |_, _| Ok(());
    let ok_preview_focused: fn(&mut App, KeyCode, KeyModifiers, &mut Vec<String>) -> Result<()> =
        |_, _, _, _| Ok(());
