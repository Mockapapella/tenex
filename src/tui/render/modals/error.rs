//! Error and success modal rendering
...
/// Render a success modal with word-wrapped message
pub fn render_success_modal(frame: &mut Frame<'_>, message: &str) {
    // Wrap the message to fit within the modal width (44 chars after padding)
    let max_line_width = 44;
    let mut lines: Vec<Line<'_>> = Vec::new();

    // Add success icon and header
    lines.push(Line::from(Span::styled(
        "✔ Success",
        Style::default()
...
mod tests {
    ...
    #[test]
    fn test_render_error_modal_short_message() -> Result<(), std::io::Error> {
        ...
        // Verify no panic occurred during render
        Ok(())
    }

    #[test]
    fn test_render_error_modal_long_message() -> Result<(), std::io::Error> {
        let backend = TestBackend::new(80, 24);
        let mut terminal = Terminal::new(backend)?;
    ...
    #[test]
    fn test_render_success_modal_short_message() -> Result<(), std::io::Error> {
        let backend = TestBackend::new(80, 24);
        let mut terminal = Terminal::new(backend)?;
    ...
    #[test]
    fn test_render_success_modal_long_message() -> Result<(), std::io::Error> {
        let backend = TestBackend::new(80, 24);
        let mut terminal = Terminal::new(backend)?;
